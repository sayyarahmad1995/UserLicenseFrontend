...
import { Loader2, AlertCircle, Calendar, Activity, Zap } from 'lucide-react';
...
const StatCard = ({ title, value, subtitle, variant = 'default', index = 0 }: { title: string; value: string | number; subtitle?: string; variant?: 'default' | 'success' | 'warning' | 'danger'; index?: number }) => {
  ...
  return (
    <div 
      className={`${bgColor} p-4 rounded-lg shadow hover:shadow-lg hover:scale-105 transition-all duration-300 animate-fade-in`}
        ...
        <div className="mb-6 animate-fade-in" style={{ animationDelay: '300ms' }}>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <StatCard 
              ...
              value={typeof health?.database === 'object' ? health?.database?.status : health?.database || 'Unknown'}
              subtitle="Connection status"
              variant={(typeof health?.database === 'object' ? health?.database?.status : health?.database) === 'up' ? 'success' : 'danger'}
              index={0}
            />
            <StatCard 
              title="Redis Status" 
              value={typeof health?.redis === 'object' ? health?.redis?.status : health?.redis || 'Unknown'}
              subtitle="Cache status"
              variant={(typeof health?.redis === 'object' ? health?.redis?.status : health?.redis) === 'up' ? 'success' : 'danger'}
              index={1}
            />
            <StatCard 
              title="API Status" 
